...
2. Phase 2: Gemini 2.5 Flashで実素材を一括OCRし、数学答案として十分に読めるかを確認する。必要ならMathpixなどのSTEM OCR APIと比較する。
...
- 手描き素材検証は `npm run ocr:handwriting -- --limit 5` で開始する。`GEMINI_API_KEY` または `GOOGLE_API_KEY` を `.env.local` に置くと、`gemini-2.5-flash` が画像OCR結果をJSON/Markdownへ保存する。
