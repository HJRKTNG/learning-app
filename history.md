...
- OCR分割時に手書き文字を跨がないよう、Geminiのbboxをそのまま使わず、`sharp` のグレースケール画像から行/列のインク密度と空白帯を解析して、上下左右の境界を空白へスナップする補正を追加。隣接問題・ページ列ガター・実インク範囲を見て、問題7/10、問題15/16、問題2/5のような混入や切れを抑えるようにした。
- 手描き素材3ページを `--limit 3 --max-blocks 3` でレイアウト検証し、続けてGemini 2.5 Flashの実OCR/補正チェックを9ブロックで実行。比較用に `StudyLockSpike/tmp/ocr-handwriting/whitespace-ocr-3pages.html` / `.md` / `.json` を生成した。
...
- ユーザーフィードバック「カード型UIはAIっぽいので嫌」「一画面に収まるように」を受けてUIを再刷新。白角丸カード＋影を全廃し、白地＋ヘアライン区切り＋エッジトゥエッジ行＋左罫線コールアウト（`Callout`）＋文字色タグ（`Tag`）のフラットなiOSスタイルに変更。全画面をスクロールなしの一画面構成にし、長文になりうる問題文・採点結果のみ内部スクロールに限定。ホームのAPI説明パネルは廃止し、オンデマンド生成は問題画面のフォールバックに集約。
- 出題設定（学校・難易度）の選択機能を追加。`src/services/studySettings.ts` に学校（中学/高校/大学受験/難関大）と難易度（基礎/標準/応用）のプリセット、トピック自由入力、永続化、生成APIリクエスト合成（`level` に学校＋難易度を合成、`difficulty` フィールドも送信）を実装。設定画面と保護者ダッシュボードの両方に `OptionSelect` セレクターを設置し、事前生成・オンデマンド生成の両方に反映。Jestテスト4件を追加。
- 出題内容（Subject/Level/Topic）の入力を開発ダッシュボードからアプリ内設定に移行し、devダッシュボードはAPI URL/Tokenのみに簡素化。`LearningPreviewState` から `showApiPanel` を削除。
- ビルド成果物 `dist/` をESLintの対象から除外（`.eslintrc.js` に `ignorePatterns` 追加）。
- `tsc`、`eslint --max-warnings=0`、Jest 14件、`vite build` の成功と、Chromeヘッドレスで主要7画面（ロール選択・学習者設定・保護者・ホーム・設定・問題・正解）が一画面に収まることを確認。
